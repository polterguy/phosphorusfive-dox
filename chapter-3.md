...
Notice, you are responsible yourselves for making sure your Ajax event actually is associated with a legal DOM event on the client side yourself. You could easily create an Ajax event called *"onxyz"* if you want to. However, this will result in non-conforming HTML, and serve no purpose for you. The same is true for attributes.
...
The **[oninit]** argument declares a lambda object, which will be executed when the widget is displayed. This lambda will be executed on the server. It allows you to create initialization logic, which will execute whatever *"initialization logic"* you wish to execute. The **[oninit]** lambda will be executed only when the widget becomes visible, and in fact, once *every time* it becomes visible. This means that if you hide a widget, for then to display it again, and keep on repeating this process - Your **[oninit]** will be executed once, every time you show your widget.
