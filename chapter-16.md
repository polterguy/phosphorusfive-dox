...
An Active Event that is an extension widget, as the above illustrates, needs to return *exactly one widget*. This widget can of course be a **[container]** type of widget though, which allows us to create any amount of internal complexity within it, as we see fit. The above widget, is probably not that useful. However, since Active Events can be parametrized, changing our lambda object as it executes, this allows us to parametrize our custom widgets, any ways we see fit.
...
Now the thing to realize, is that the above **[sys42.settings.set]** Active Event, and the **[sys42.settings.get]** Active Event, are helper events from System42, that allows you to retrieve and change settings values, on a per app/user basis. The above extension widget hence, allows you to easily create a widget, for every time you want to change some color setting for your application. Hence, you have created a highly reusable widget, which you can use, every time some user wants to change a setting, which is some color of some sort. And the widget itself, will automatically take care of persisting the new settings values to the database.

You can see this, if you change its value, for then to refresh the page. At which point, it will fetch the value automatically from the database.
...
Another example is p5.auth, which authenticates and authorize users in P5. Although relatively safe and secure, this component is still a little bit *"naive"* in its implementation - Particularly in regards to how it handles roles. If you wanted to, you could easily replace the entirety of p5.auth, with your own auth component. And as long as you created Active Events having the same names, taking the same set of arguments, and returning (at least) the same set of values - Nothing would break. You basically always have the *"adapter patter"* at your disposal, implicitly implemented, on everything you create.
...
Another concern, is that the above component *cannot* be implemented multiple times, on the same page, for different CRUD objects. This is because of that our widget's lambda events, doesn't check to see which datagrid we update, or page. Hence, whenever you page one datagrid, you will page *all* datagrids on your page. This too is left as an exercise for the reader, if such features are necessary.
...
Interestingly though, our dependencies for our datagrid, does *not* include neither the *"create"* operation, nor the *"read"* operations from our previously created CRUD database layer. This is because that the creation of new records, is contained within our *"Create"* button, which is not a part of our datagrid widget. In addition, we retrieve our items during its *"read"* invocation, through a lambda callback, supplied during creation of our datagrid. And since this is not a part of the code for the datagrid, our datagrid is not in fact dependent upon our *"CRUD read"* operation.

If we were to further generalize our datagrid, and improve it, some of the things we'd probably want to do, would be to make it completely independent upon all of our previously created database CRUD layers, and further rely upon the lambda callback features, to pass in *"callbacks"* to do all the things related to *"non-GUI"*. This would create a better and more scalable datagrid, in addition to make it independent upon its underlaying *"model"*.

Separating the *"model"* and the *"view"* are often considered to be *"good architecture"*. However, also these tasks, are left as exercises for the reader, if you wish to further improve the datagrid.
...
If you would like to improve the above datagrid's architecture though, here is a list of things you'd probably consider to improve it.
